		...
		while(true) {
			...
			if(menu == 1) {
				...
				for(int i=0; i<list.size(); i++) {
					HashMap<String, Object> user = list.get(i); // 해시맵으로 꺼낸다는 의미
					if(name.equals(user.get("name"))) { // 문자열엔 ==쓰면안되서 .
						System.out.print("자바 : " + user.get("java")); // 유저라는맵안에 자바라는키에 담겨있다는뜻
						System.out.print("db : " + user.get("db"));
						System.out.print(", html : " + user.get("html"));
						searchFlg = true; // 밑에를 트루로바꿔주는멘트
						break; // 학생을 찾았을때 빠져나가는 멘트 쓸데없는걸안하게막는거
					}
				}
				if(!searchFlg) { // searchFlg == false 여도되지만 이미하나만써도 false
